...
#SBATCH --output=/home/jepe/EcoGenetics/people/Jeppe_Bayer/steps/03_init_analysis_files/full_vcf-%j.out
...
-p 100 \
-f "$RG" \
-v "$WD"/"$data"/"$(basename "$SD")"/"$(basename "$sample")"/"$(basename "$sample")"_full.vcf \
--pooled-discrete \
