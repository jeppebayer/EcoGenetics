...
-dir "$repmodrun" \
-xsmall \
-lib "$repmod"/"$(basename "$(dirname "$reference_genome")")"-families.prefix.fa \
"$repbaserun"/"$(basename "$reference_genome")".masked

rename 's/.masked././' ./*

# Make GFF 3 of repeat regions from RepeatMasker output file
repeatmasker_out="$repmodrun"/"$(basename "$reference_genome")".out
...
awk \
    ...
    "$repeatmasker_out" \
    > "$(dirname "$repeatmasker_out")"/"$filename".repeats_rm.gff
